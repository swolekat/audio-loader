 ...
 */
const load = async (source, options, defVal) => {
  var loader =
    // Basic audio loading
  ...
  let promise;
  if(loader) {
    promise = loader(source, opts)
  } else {
    if(defVal){
      promise = Promise.resolve(defVal)
    } else {
      promise = Promise.reject('Source not valid (' + source + ')')
    }
  }
...
const decodeBuffer = async (array, options) => {
  const decode = await import('audio-decode');
  const decoded = await decode.default(array);
  return decoded;
}
